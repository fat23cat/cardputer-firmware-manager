...
def _print_staged(selected: List[str]) -> None:
    print("staged successfully; exit CRUB usbsd, then run:")
    for app_id in selected:
        print(f"  up{app_id}")


def _doctor(catalog: Mapping[str, Any], sd_root: Optional[Path]) -> None:
    ...
    if sd_root:
        ...
        if not sd_root.is_dir():
            raise FirmwareError(f"SD root is not mounted: {sd_root}")
        for app_id, app in catalog["apps"].items():
            image = sd_root / app["sd_path"]
            if image.exists():
                validate_image(app_id, image, app["partition_size"])
    ...
    if args.command == "local":
        ...
        for app_id in selected:
            ...
            if args.build:
                ...
                subprocess.run(app["build_command"], cwd=repository, check=True)
